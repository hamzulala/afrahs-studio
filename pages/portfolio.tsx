...
const Portfolio = (props: Props) => {
  ...
  return (
    <div className='flex  items-center justify-center h-screen ease-in duration-300' style={{ backgroundColor: `${primaryColor}` }}>
        <Header primaryColor={primaryColor} secondaryColor={secondaryColor}/>
        {/*
         * Should be a dynamic list of all projects
         */}
        <div className='absolute columns-2 md:columns-3 lg:columns-4 p-4 top-20 ease-in duration-300' style={{ backgroundColor: `${primaryColor}` }}>
          {projects.map((project) => (
            <ProjectImgLink key={project.id} project={project} handleProjectHover={handleProjectHover}/>
          ))}
        </div>
    </div>
  )
